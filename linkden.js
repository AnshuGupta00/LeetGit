...
document.getElementById('copyAndOpen').addEventListener('click', async () => {
  ...
  linkedinStatusEl.textContent = 'Opening LinkedIn...';

  const shareUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(lastProblemLink || 'https://leetcode.com')}`;
  const newTab = await chrome.tabs.create({ url: shareUrl });
  ...
  try {
    ...
    linkedinStatusEl.textContent = result
      ? 'Text drafted — click into the image area and press Ctrl+V, then hit Post'
      : "Couldn't auto-fill text. Paste the text manually, then Ctrl+V for the image";
  } catch (err) {
    console.warn('Auto-fill failed:', err);
    linkedinStatusEl.textContent = "Couldn't auto-fill text. Paste the text manually, then Ctrl+V for the image";
  }
}); 